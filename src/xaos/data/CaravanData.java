...
public class CaravanData implements Externalizable {
    ...
    /**
     * Chequea si la caravana est� lista para trade Cambia el flag "ready" de la
     * misma (tanto true como false)
     *
     * @return true si hay que eliminar la caravana (pq se fu�)
     */
        ...
        switch (getStatus()) {
            case STATUS_COMING:
                ...
                if (le != null) {
                    ...
                    if (lemi != null && lemi.getCaravan() != null) {
                        if (le.getPath().size() == 0) {
                            ...
                            if (iZoneID != 0) {
                                ...
                                // No est� en una zona y no tiene path. A buscar zona o a pirarse
                                Zone zone;
                                ...
                                if (cmi != null) {
                                    ...
                                    if (alDestinations == null || alDestinations.size() == 0) {
                                        ...
                                        // Si llega aqu� es que nanay de la china
                                        setStatus(STATUS_LEAVING);
                                    }
                                } else {
                            ...
                            // Tiene camino, a saber pq se ha llamado a esta funci�n
                        }
                    } else {
                        // Raro, por si acaso nos piramos
                        setStatus(STATUS_LEAVING);
                    ...
                    // Rar�simo, no existe en el mundo
                    setStatus(STATUS_LEAVING);
                }
                break;
    ...
    public boolean confirmTrade() {
        ...
        // Miramos si ya se puede soltar alg�n item
    ...
    private boolean checkItemToDrop() {
        ...
        // Seg�n la pasta del pueblo miramos si puede soltar objetos
        int iWorldCoins = Game.getWorld().getCoins();
        if (iWorldCoins > 0) {
            // Hay pasta, miramos si hay alg�n item que pueda soltar
            int iIndexItem = -1;
            ...
            if (iIndexItem != -1) {
                ...
                if (iIndexRealItem != -1) {
                    ...
                    // Item no encontrado �?�?�? rar�simo, lo eliminamos de la lista de to-buy
                    getMenuCaravanToBuy().getItems().remove(iIndexItem);
                }
            }
        }
     ...
     *
     * @param item
     * @return true si la transacci�n es correcta (deber�a)
     */
    public boolean itemCarried(Item item) {
        ...
        if (iIndexItem != -1) {
            ...
            // Miramos si la caravana puede soltar m�s cosas
            checkItemToDrop();
    ...
    private Point3DShort getDropPoint(Point3DShort p3dIniPoint, ItemManagerItem imi) {
        ...
        // Si est� en una zona (deber�a) miraremos los puntos de la misma
        ...
        // No est� en zona o no hay places disponibles, miramos un radio de 8
        if (!bCellFound) {
