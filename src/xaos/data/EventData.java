...
public class EventData implements Externalizable {
     ...
     * @param singleLiving Si se pasa una living s�lo intenta aplicarlo a ella,
     *                     no a todos
     */
    public void launchEffects(EventManagerItem emi, ArrayList<String> effectsList, LivingEntity singleLiving) {
        if (emi.getTargets() != null && !emi.getTargets().isEmpty()) {
            ...
            if (effectsList != null && !effectsList.isEmpty()) {
                ...
                // Por si acaso han cambiado el xml, comprobamos tama�os y esas cosas
                    ...
                    if (emi.getTargetsRandomCell() == null || emi.getTargetsRandomCell().isEmpty()) {
                        ...
                        while (iNumCells > 0) {
                            ...
                            if (cell.isDiscovered() && cell.getLivings() != null && !cell.getLivings().isEmpty()) {
                                ...
                                // Cell con livings
                                // Por cada livings que est� en el target le meteremos todos los efectos
                                LivingEntityManagerItem lemi;
                                LivingEntity le;
                                for (int l = 0; l < cell.getLivings().size(); l++) {
                                    ...
                                    if (lemi != null) {
                                        if (emi.getTargetsHateData().isHate(le)) {
                                            // Miramos si es un target v�lido
                                            int iTargetIndex = -1;
