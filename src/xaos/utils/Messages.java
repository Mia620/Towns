        ...
        if (sModName == null) {
            ...
            if (!fUserFolder.exists() || !fUserFolder.isDirectory()) {
                ...
                if (sLanguage.equals("en") && sCountry.equals("US")) {
                    sModLanguagePath = fUserFolder.getAbsolutePath() + FileSystems.getDefault().getSeparator() + Game.MODS_FOLDER1 + FileSystems.getDefault().getSeparator() + sModName + FileSystems.getDefault().getSeparator() + Towns.getPropertiesString("DATA_FOLDER") + "/languages/messages.properties"; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$
                } else {
