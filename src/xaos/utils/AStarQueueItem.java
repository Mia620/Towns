...
public final class AStarQueueItem {
     ...
     * que toque con la misma Esto evita diagonales raras al andar El m�todo
     * tiene que ser r�pido para no perder performance
     *
     * @param alPath
     */
    public static void preSmoothPath(ArrayList<Point3DShort> alPath) {//, int livingType) {
        ...
        // Empieza la fiesta
        // Miramos la primera Z, porque �ste m�todo no funciona para Z distintas
        int startIndex = 0;
...
//				} else {
                    // Path demasiado corto, no hay que tocar el �ndice
                }
        ...
        // Al salir de aqu� smootheamos el �ltimo tramo
...
//			if (alBresenham != null && alBresenham.size () > 2) {
                ...
                // Miramos el tama�o de lo que tenemos en el path
                int subPathSize = (iIndexEnd - iIndexStart);
                if (alBresenham.size() <= (subPathSize + 1)) {
                    // Linea del mismo tama�o o m�s corta, primero sustituimos el path
                    ...
                    if (alBresenham.size() < subPathSize) {
                        // Linea m�s corta, hay que borrar lo sobrante del path
    ...
    /**
     * Continua (o empieza) la b�squeda A* hasta un m�ximo de "iMaxIterations"
     * iteraciones.
     *
     * @param iMaxIterations
     * @return el n�mero de iteraciones efectuadas
     */
    public int search(int iMaxIterations) {
        // Miramos si las 2 celdas a�n est�n en la misma zona (por si acaso alguien ha digado o hecho algo mientras buscaba)
        ...
        // Miramos si es una continuaci�n
        //boolean bContinue = getClosedList () != null;
        AStarNodo nodo = null;
        if (isNewItem()) {
            setNewItem(false);
            // Miramos las casillas cercanas al punto final y origen, empezaremos la b�squeda por el que tenga m�s
            // restricciones, o sea, el que tenga m�s cells-NO-Allowed
            // Segun que ruta podemos mejorar la velocidad de b�squeda en un grado 10.000 a 1 (aprox.)
            ...
            // A�adimos el primer nodo en la lista abierta
        ...
        while (iIterations < iMaxIterations && !openList.isEmpty()) {
            iIterations++;
            nodo = openList.remove(0); // Pillamos el 1er nodo (la lista ya estar� ordenada)
            if (nodo.x == endPoint.x && nodo.y == endPoint.y && nodo.z == endPoint.z) { // Hemos encontrado el destino
                ...
                // Diagonales
                // Es m�s r�pido (casi el doble) si no las miramos, aunque el camino puede quedar ortop�dico
                // Aunque eso se soluciona con la optimizaci�n de caminos que hacemos una vez encontrado el mismo
                ...
                // pero por la mitad de ese camino si que hab�a.
        ...
        if (bEncontrado) {
            ...
            //setClosedList (null);
            // Tenemos 1 camino posible a destino, optimizamos la ruta para que no sea ortop�dica
...
//			} else {
    ...
    /**
     * M�todo interno del A*
     */
    private boolean checkNewNode(AStarNodo currentNode, AStarBinaryHeap alListaAbierta, int x, int y, int z) {
        // Miro la closedList[0][0].length, no fuera que el levels discovered creciera mientras busca un camino y decidiera buscar por ah�
