...
public final class UtilsAL {
    ...
    private static boolean loadAudio(String sKey, String sFile) {
        ...
        if (audio == null) {
            Log.error(Messages.getString("UtilsAL.5") + sFilePath + "]", "UtilsAL"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
            return false;
        }
    ...
    public static void initAL(int musicVolume, int FXVolume) {
        if (Game.isMusicON() || Game.isFXON()) {
            if (!isOpenALON()) {
                if (!loadALData()) {
                    Log.error(Messages.getString("UtilsAL.8"), "UtilsAL"); //$NON-NLS-1$ //$NON-NLS-2$
                    Game.exit();
                }
                setOpenALON(true);
