...
public final class UtilsAL {
     ...
     *
     * @param sKey
     * @param sFile Fichero, si se le pasa null (o vac�o) no pasa nada, mete
     *              null en la hash
     * @return
     */
    private static boolean loadAudio(String sKey, String sFile) {
        ...
        if (audio == null) {
            Log.log(Log.LEVEL.ERROR, Messages.getString("UtilsAL.5") + sFilePath + "]", "UtilsAL"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
            return false;
        }
    ...
    public static void initAL(int musicVolume, int FXVolume) {
        if (Game.isMusicON() || Game.isFXON()) {
            if (!isOpenALON()) {
                if (!loadALData()) {
                    Log.log(Log.LEVEL.ERROR, Messages.getString("UtilsAL.8"), "UtilsAL"); //$NON-NLS-1$ //$NON-NLS-2$
                    Game.exit();
                }
                setOpenALON(true);
