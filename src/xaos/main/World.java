...
import xaos.tiles.terrain.special.*;
import xaos.utils.*;
...
public final class World implements Externalizable {
    ...
    public final static int FLUIDS_MOVED_PER_INVOCATION = 64; // Max fluidos a procesar por turno (tambi�n se usa para la evaporaci�n)
    ...
    public static ArrayList<Point3DShort> fluidEvaporation = new ArrayList<>(); // Fluidos con fuerza 1 que van a evaporarse cada X tiempo
    private static ArrayList<Integer> citizenIDs = new ArrayList<>();
    private static ArrayList<Integer> soldierIDs = new ArrayList<>();
    private static ArrayList<Integer> heroIDs = new ArrayList<>(2);
    private static ArrayList<Building> buildings;
    private static HashMap<Integer, Item> items;
    private static HashMap<Integer, ArrayList<String>> itemsText;
    private static final ArrayList<Integer> fallItemList = new ArrayList<>();
    private static HashMap<Integer, LivingEntity> livingsDiscovered = new HashMap<>();
    private static HashMap<Integer, LivingEntity> livingsUndiscovered = new HashMap<>();
    ...
    // ID M�ximo (current) de los entities (livingentities, items, edificios, special)
    ...
    private ArrayList<Point3DShort> exploringHotPoints = new ArrayList<>();
    private int maxHeroXP = 0;
    private ArrayList<LivingEntity> oldHeroes = new ArrayList<>(2);
    private ArrayList<String> oldHeroesDied = new ArrayList<>(2);
    private CaravanData currentCaravanData;
    private HashMap<String, Integer> enemiesKilled;
    private ArrayList<Projectile> projectiles;
    private ArrayList<Integer> itemsToBeHauled = new ArrayList<>();
    ...
    private transient int indexViewCitizen; // Indica el n�mero de aldeano en la lista global de aldeanos que se est� "viendo"
    private transient int indexViewSoldier; // Indica el n�mero de soldado en la lista global de aldeanos que se est� "viendo"
    private transient int indexViewHero; // Indica el n�mero de h�roe en la lista global de h�roes que se est� "viendo"
     ...
     *
     * @param iASZI
     * @param iMinLevelToCheck       Nivel m�nimo donde puede spawnear
     * @param bUseBorderPointIfFails Si no encuentra punto buscar� un punto normal en los bordes del mapa
     * @return
     */
    public static Point3DShort getRandomUndergroundPoint(int iASZI, int iMinLevelToCheck, int maxLevelDiscovered, boolean bUseBorderPointIfFails) {
        ...
        while (iIndexLevel > iMinLevelToCheck) {
            // Miramos 200 randoms como m�ximo (MAP_WIDTH)
            int iTries = MAP_WIDTH;

            while (iTries > 0) {
                iTries--;

                y = switch (Utils.getRandomBetween(1, 4)) {
                    case 1 -> {
                        // Norte
                    ...
                    case 3 -> {
                        // Este
                        x = MAP_WIDTH - 1;
                        yield Utils.getRandomBetween(0, (MAP_HEIGHT - 1));
                    }
                    default -> {
                        // Oeste
                        x = 0;
        ...
        // Si llega aqu� es que no hemos encontrado una casilla a random, recorremos el mundo a manija
        ...
        // Si llega aqu� es que nada de nada
    ...
    /**
     * Devuelve un punto a random, que estar� en los bordes y en el nivel 0, bas�ndose en el ASZI. Se usa en la llegada de inmigrantes/heroes.
     *
     * @param iASZID
     * @return
     */
    public static Point3DShort getRandomBorderPoint(int iASZI) {
        ...
        // Si as� no encontramos nos recorreremos todo el mapa
        ...
        // Si llega aqu� es que los randoms no han funcionado, nos recorremos todo el mundo
        // Norte/sur
    ...
    /**
     * Mueve el fluido de una celda origen (se asume que hay fluido ah�) a una destino, y hace las cosas necesarias (borrar items de celdas, minimapa, ...)
     *
     */
    public static boolean moveSingleFluid(Cell cellSource, Cell cellDestination, boolean bPreloading, Point3DShort p3dSource, Point3DShort p3dDestination, ArrayList<Point3DShort> alNewPoints) {
        ...
        if (!bMovedFromFluidToFluid) {
            ...
            // Celda destino ya ten�a fluidos
    ...
    public static void deleteCellAll(Cell cell, boolean bFluids) {
        ...
        // Punto de stockpile borrado (se borra s�lo si fluids)
        if (bFluids) {
        ...
        if (!cell.isEmpty()) {
            if (cell.hasItem()) {
                ...
                // Cualquier otra cosa (s�lo fluids, a revisar)
                if (bFluids) {
    ...
    /**
     * A�ade enemies al mundo
     *
     * @param numEnemies N�mero de malos
    ...
    public static boolean addSiegeEnemies(Point3DShort p3dSpawnPoint, int numEnemies, int level, byte bSiegeType, int iWaitTurns) {
        // A�adimos enemigos en el punto indicado
    ...
    public static boolean addSiegeEnemies(Point3DShort p3dSpawnPoint, int numEnemies, LivingEntityManagerItem lemi, byte bSiegeType, int iWaitTurns) {
        ...
        if (bSiegeType == SiegeData.SIEGE_ROBBERY) {
            ...
            if (!bSiege) {
                // Si a�n no hay siege, miramos que existan containers
                if (Game.getWorld().getContainers() != null && !Game.getWorld().getContainers().isEmpty()) {
                    bSiege = true;
                }
            }
    ...
    // // Si al cabo de X intentos no se consigue uno que no est� en la lista, se recorre a manija la lista, uno a uno
    //
    // String sGodID;
    // GodManagerItem gmi;
    ...
    // }
    //
    // // Si llega aqu� ya lo tenemos
    // gods.add (gmi.getGodDataInstance ());
    // return;
    // }
    // }
    //
    // // Si llega aqu� nos recorreremos la lista de gods a manija
    ...
    public void generateAll(String sCampaignID, String sMissionID) {
        ...
        if (TownsProperties.DEBUG_MODE) {
            ...
            Log.debug(sLog, getClass().toString());
        ...
        // Exploring hotpoints
        exploringHotPoints = new ArrayList<>();
        maxHeroXP = 0;
    ...
    public void moveFluidsPreNewGame() {
        ...
        while (iCounter > 0 && !fluidCellsToProcess.isEmpty()) {
    ...
    public void refreshTransients() {
        ...
        for (short z = 0; z < MAP_DEPTH; z++) { // Importante poner la Z aqu� para que el iMaxDiscovered no se lie
    ...
    public void checkGlobalEvents() {
        ...
        while (iIndex >= 0) {
            ...
            // Primero de todo comprobamos que a�n tenga los prerequisitos
    ...
    /**
     * A�ade un evento a la lista, comprueba los prerequisitos y elimina los eventos a los que es immune
     *
     * @param emi
     */
    public boolean addEvent(EventManagerItem emi) {
        ...
        for (EventData event : events) {
            if (event.getEventID().equals(emi.getId())) {
                return false;
            }
        }

        // Si llega aqu� es que no existe en la lista actualmente
        // Comprobamos los prerequisitos, todos deben existir
        if (!containsEventPrerequisites(emi.getId())) {
            return false;
        }

        // Comprobamos que alg�n efecto no lo tenga como immune
        ...
        // Si llega aqu� es que los prerequisitos est�n ok, creamos una instancia y la metemos
        // Miramos la posici�n donde debe ir
        int iIndex = -1;
    ...
    /**
     * Comprueba si un evento est� en la lista de eventos
     *
     * @param sEvent
     * @return
     */
    private boolean containsEvent(String sEvent) {
        for (EventData event : events) {
            if (event.getEventID().equals(sEvent)) {
                return true;
            }
        }
    ...
    /**
     * Comprueba si un evento puede a�adire por los immunes actuales
     *
     * @param sEvent
     * @return
     */
    private boolean containsEventImmunization(String sEvent) {
        ...
        for (EventData event : events) {
            emi = EventManager.getItem(event.getEventID());
    ...
    /**
     * A�ade una stockpile a la lista y setea el ID de stockpile en todas las celdas involucradas
     *
     * @param pile
     */
    public void addStockPile(Stockpile pile) {
        ...
        Point3DShort p3d;
        for (Point3DShort point3DShort : alp3d) {
            p3d = point3DShort;
    ...
    /**
     * A�ade un container si no existe con anterioridad
     *
     * @param iItemID
     */
    public void addContainer(int iItemID) {
        // Miramos que no exista ya (por si lo est�n moviendo de sitio)
        Container container;
        for (int i = 0; i < getContainers().size(); i++) {
            container = getContainers().get(i);
            if (container.getItemID() == iItemID) {
                // Ya existe, quiz� lo ha movido de sitio, por lo que setearemos las coordinates de los items de dentro
                Item itemContainer = getItems().get(iItemID);
    ...
    /**
     * A�ade una zona a la lista y setea el ID de zona en todas las celdas involucradas
     *
     * @param zone
     */
    ...
    /**
     * Setea el array [MAP_WIDTH][MAP_HEIGHT][MAP_DEPTH] de ints indicando por cada celda el n�mero de proyectiles
     */
    ...
    /**
     * Genera aldeanos en un punto de partida com�n
     */
    private void generateCitizens() {
        ...
        // Buscamos un punto de llegada v�lido (10.000 intentos)
        // int outsideIndex = 1;
        int trys = 0;
        ...
        if (trys == 0 || p3d == null) {
            ...
            Log.debug(Messages.getString("World.4"), getClass().toString()); //$NON-NLS-1$
            return;
        }
    ...
    private boolean validStartingPoint(Point3DShort p3d) {
        // Miraremos que tenca una cuadr�cula de 17x17 sin agua
        ...
        // Tambi�n miramos si la celda es ok para ellos
...
//			System.out.println ((l - System.currentTimeMillis ()) + "ms");
    ...
    private void generateDungeons(DungeonData dungeonData) {
        ...
        for (MonsterData alMonster : alMonsters) {
            monsterData = alMonster;
            int iNumber = monsterData.getNumber();

            ArrayList<LivingEntityManagerItem> alMonsterList = null;
            // Metemos a random el monster indicado tantas veces como est� indicado
            ...
            for (int n = 0; n < iNumber; n++) {
                if (alMonsterList == null || alMonsterList.isEmpty()) {
                    // Enemigo fijo
                ...
                if (lemi == null) {
                    Log.error(Messages.getString("World.5") + monsterData.getId() + "]", getClass().toString()); //$NON-NLS-1$ //$NON-NLS-2$
                    Game.exit();
                }
    ...
    public void nextTurn() {
        // Cursores (si no est� sacando el panel de typing)
        ...
        // }
        // Demo version, codificado de forma rara para evitar que se toque con un editor hexadecimal o as�
        ...
        while (iIndex >= 0) {
            if (iIndex < events.size() && events.get(iIndex).nextTurn()) { // Cuidado, que al hacer deletes pueden pasar cosas raras (no deber�a)
        ...
        // Si alg�n camino no se ha encontrado hacemos un full check
        if (isRecheckASZID()) {
    ...
    /**
     * Recorre todos los aldeanos y les modifica la happiness seg�n lo que est�n haciendo / viendo
     */
    private void modifyHappiness() {
        Citizen citizen;
        // Cada hora modificamos la happiness seg�n lo que est�n haciendo/viendo en ese momento
        for (int i = 0; i < getCitizenIDs().size(); i++) {
    ...
    private void modifyHappiness(Citizen citizen) {
        ...
        // Modificador por LOS (s�lo si el idle y work counters no son 0)
        if (citizen.getCitizenData().getHappinessWorkCounter() != 0 && citizen.getCitizenData().getHappinessIdleCounter() != 0) {
            ArrayList<Integer> alItemsHappy = new ArrayList<>();
            ...
            if (!alItemsHappy.isEmpty()) {
                citizen.getCitizenData().setHappiness(citizen.getCitizenData().getHappiness() + alItemsHappy.get(Utils.getRandomBetween(0, alItemsHappy.size() - 1)));
            }
        }
    }

    /**
     * Mira si se lanza alg�n efecto random
     */
    ...
    private void checkImmigrants() {
        ...
        if (!getCitizenIDs().isEmpty()) {
            ...
            // Miramos que haya happiness suficiente, basada en el n�mero de aldeanos
            ...
            if (iHappiness > happinessMin) {
                ...
                // Miramos si hay Zones personales libres, en otro caso no vendr�n
                int iASZID;
                ArrayList<Integer> alASZID = new ArrayList<>();
                ArrayList<Integer> alZonesID = new ArrayList<>();
                ArrayList<Point3DShort> alZonesPoint = new ArrayList<>();
                for (int i = 0; i < getZones().size(); i++) {
                    if (ZoneManager.getItem(getZones().get(i).getIniHeader()).getType() == ZoneManagerItem.TYPE_PERSONAL && ((ZonePersonal) getZones().get(i)).getOwnerID() == -1) {
                        ...
                        // Buscamos un punto ASZID v�lido en la zona
                ...
                if (!alASZID.isEmpty()) {
                    ...
                    for (int i = 0; i < alASZID.size(); i++) {
                        // Buscamos el sitio donde saldr�n, nos basamos en el A*ZI de las Zones libres
                        if (p3dArrival == null) {
                            p3dArrival = getRandomBorderPoint(alASZID.get(i));
                        } else {
                            // Ya tenemos punto de antes, miramos si es la misma zona
                            // �sto se hace para que los aldeanos vengan del mismo sitio y no dispersos
                            if (getCell(p3dArrival).getAstarZoneID() != alASZID.get(i)) {
                                // Distinto Zone ID, calculamos nuevo punto
                                p3dArrival = getRandomBorderPoint(alASZID.get(i));
                            } // Else, mismo zone ID, as� que mantenemos el punto de arrival
                        }
                        ...
                        if (p3dArrival != null) {
                            ...
                            if (citizen != null) {
                                iImmigrantsOK++;
                                ZonePersonal.assignZone(citizen, alZonesID.get(i));
                                // Le a�adimos metemos la zona
                                updateHappiness(80);
    ...
    private void checkHeroesLeave() {
                    ...
                    if (prerequisite.getId() == HeroPrerequisite.ID_MIN_CITIZENS) {
                        ...
                        if (hero.getCitizenData().getZoneID() == -1) {
                            ...
                            if (zone == null) {
                                ...
                                if (!zone.getPoints().isEmpty() && zone.getPoints().get(0).z < World.MAP_NUM_LEVELS_OUTSIDE) {
                                    bPrerequisitesOK = false;
                                    break;
                                }
                            }
                        ...
                        if (hero.getCitizenData().getZoneID() == -1) {
                            ...
                            if (zone == null) {
                                ...
                                if (!zone.getPoints().isEmpty() && zone.getPoints().get(0).z > (MAP_NUM_LEVELS_OUTSIDE - prerequisite.getValueInt())) {
                                    bPrerequisitesOK = false;
                                    break;
                                }
                            }
        ...
        if (alLeavingHeroes != null) {
            ...
            for (Integer alLeavingHero : alLeavingHeroes) {
                iID = alLeavingHero;
                hero = (Hero) World.getLivingEntityByID(iID);
                if (hero != null) {
    ...
    /**
     * Mira si llegan h�roes. En ese caso los mete
     */
    public void checkHeroesCome() {
        ...
        // Antes de nada miramos si tenemos el m�ximo de h�roes, nos basamos en el n�mero de aldeanos
        ...
        LivingEntityManagerItem lemi;
        boolean bOldHeroToCome = !getOldHeroes().isEmpty() && Utils.getRandomBetween(1, 20) == 1;
        if (bOldHeroToCome) {
            // Miramos si puede llegar un h�roe de los que ya se han ido
        ...
        if (lemi != null) {
            // Antes de mirar los prerequisitos miramos que no sea un h�roe �nico (con nombre fijo), en ese caso miramos que no lo tengamos ya en el mundo o en la lista de h�roes que se han ido
            if (lemi.getName() != null && (lemi.getNamePoolTag() == null || lemi.getNamePoolTag().isEmpty())) {
                // H�roe con nombre �nico, miramos que no tengamos uno de estos ya en el mundo (o en los old heros)
                Hero hero;
                ...
                for (int i = 0; i < World.getHeroIDs().size(); i++) {
                    ...
                    if (hero != null) {
                        ...
                        if (lemiWorld != null && lemiWorld.getName() != null && lemiWorld.getName().equalsIgnoreCase(lemi.getName())) {
                            // Mismo h�roe, pos ala, pacasa, no viene
                            return;
                        }
                    }
                }

                // Si llega aqu� es que el nuevo h�roe no est� en el mundo, miramos si est� en los old heros
                if (!bOldHeroToCome) {
                    ...
                    for (int z = 0; z < getZones().size(); z++) {
                        if (ZoneManager.getItem(getZones().get(z).getIniHeader()).getType() == ZoneManagerItem.TYPE_HERO_ROOM && ((ZoneHeroRoom) getZones().get(z)).getOwnerID() == -1) {
                            ...
                            if (!zoneHero.getPoints().isEmpty()) {
                                ...
                                // Buscamos un punto que tenga ASZID v�lido
                                ...
                                Log.error(Messages.getString("World.0"), getClass().toString()); //$NON-NLS-1$
                            }
                        }
                    }
            ...
            if (bPrerequisitesOK) {
                // Todo OK, h�roe tiene que aparecer
                ...
                if (p3dDestinationPoint == null) {
                    // No hay punto de room, metemos al h�roe en el mismo ASZID que el primer aldeano
                    if (!getCitizenIDs().isEmpty()) {
                ...
                if (p3dDestinationPoint != null) {
                    // Tenemos punto de destino, metemos al h�roe y lo movemos a ese punto
    ...
    private boolean isFreeRoomItemsOK(ArrayList<HeroPrerequisite> alPrerequisites, ZoneHeroRoom zoneHeroRoom) {
        ...
        String sItems = null;
        for (HeroPrerequisite hPre : alPrerequisites) {
        ...
        // Hay items que comprobar, all� vamos
        ...
        while (tokenizer.hasMoreTokens()) {
            ...
            if (imi != null) {
                // Item v�lido, a ver si hay alguno en la zona
                ArrayList<Point3DShort> p3ds = zoneHeroRoom.getPoints();
                Cell cell;
                Item item;
                for (Point3DShort p3d : p3ds) {
                    cell = getCell(p3d);
                    item = cell.getItem();
                ...
                Log.error(Messages.getString("World.24") + " [" + sItem + "]", getClass().toString()); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
            }
            return false;
        }
    ...
    private void checkHeroesFriendships() {
        // Para cada h�roe miramos si obtien alg�n nuevo amigo
        ...
        for (int i = 0; i < (iNumHeroes - 1); i++) {
            for (int h = (i + 1); h < iNumHeroes; h++) {
                ...
                if (hero.getHeroData().getMinTurnsToStay() <= 0) {
                    iPossibleFriendID = World.getHeroIDs().get(h);
                    if (!hero.getHeroData().getFriendships().contains(iPossibleFriendID)) {
                        // Si no tiene amigos o si no tiene al posible amigo, comprararemos moral y quiz� se har�n amigos
                        ...
                        if (possibleHeroFriend.getHeroData().getMinTurnsToStay() <= 0) {
                            ...
                            if (iMoraleDifference <= 5) {
                                ...
                                hero.getHeroData().getFriendships().add(iPossibleFriendID);

                                // Y viceversa
                                if (!possibleHeroFriend.getHeroData().getFriendships().contains(iID)) { // �sto deber�a cumplirse siempre
                                    possibleHeroFriend.getHeroData().getFriendships().add(iID);
                                }
                            }
                        }
                    }
    ...
    public void checkCaravansCome() {
        ...
        // Obtenemos una lista por si hay m�s de una
        Zone zone;
        ...
        // Tenemos los 2 puntos, metemos la living y que vaya para all�
    ...
    public void checkSiege(EventManagerItem emi) {
        ...
        // 3 sieges cada 360 horas (esto sin contar la protecci�n de soldados)
        int iSiegeRandom;
        if (emi == null) {
        ...
        ArrayList<Integer> alNumEnemies = new ArrayList<>();
        if (emi == null || emi.getSiegeSize() == null || emi.getSiegeSize().isEmpty()) {
            // Calculamos los puntos de siege
            int iSiegePoints = calculateSiegePoints();

            // Miramos el tipo de siege, normal o peque�a
            if (iSiegeRandom > 1) {
                // Peque�a
        ...
        if (iSiegeRandom == 1 || iSiegeRandom == 2 || (emi != null && emi.isSiegeUnderground())) {
            ...
            // Pueden aparecer underground (20% y algunos niveles m�s descubiertos
        ...
        for (int i = 0; i < alNumEnemies.size(); i++) {
            if (emi == null || emi.getSiegeLivings() == null || emi.getSiegeLivings().isEmpty()) {
                if (addSiegeEnemies(p3dSpawnPoint, alNumEnemies.get(i), (i + 1), siegeType, iWaitTurns)) {
                    bSiegeOK = true;
                }
            } else {
                LivingEntityManagerItem lemi = LivingEntityManager.getItem(emi.getSiegeLivings().get(i));
                if (lemi != null && addSiegeEnemies(p3dSpawnPoint, alNumEnemies.get(i), lemi, siegeType, iWaitTurns)) {
                    bSiegeOK = true;
                }
            }
        }
    ...
    /**
     * Copia de checkSiege, pero sin miramientos.... funci�n admin, mete la siege sin lanzar dados para ver si toca
     */
    ...
    public int calculateSiegePoints() {
        ...
        if (getNumHeroes() > 0) {
            ...
            LivingEntity le;
            for (Integer alHero : alHeroes) {
                le = getLivingEntityByID(alHero);
        ...
        // Modificador seg�n la dificultad the siege
    ...
    // if (gmi != null) {
    // // Primero de todo actualizamos las horas desde el �ltimo evento
    ...
    // }
    //
    // // Si hay m�s like que dislike el status sube, en caso contrario baja
    ...
    public void keyPressed(int tecla, int fn) {
        ...
        } else if (fn == UtilsKeyboard.FN_NEXT_HERO) { // Siguiente h�roe
            CommandPanel.executeCommand(CommandPanel.COMMAND_NEXT_HERO, null, null, null, null, 0);
        } else if (fn == UtilsKeyboard.FN_PREVIOUS_HERO) { // Anterior h�roe
    ...
    private void moveFluids(boolean bPreloading) {
        ...
        // Tambi�n si el contador de fluidos llega a 128 (por ejemplo)
        if (!bPreloading) {
            if (fluidsMoved) {
                fluidMovedCounter++;
            }
        ...
        if (fluidCellsToProcess.isEmpty()) {
            return;
        }
        ...
        ArrayList<Point3DShort> alNewPoints = new ArrayList<>();

        int iCounterMoved = 0;
        int iCounterNotMoved = 0;
        while (iCounterMoved < FLUIDS_MOVED_PER_INVOCATION && iCounterNotMoved < FLUIDS_NOT_MOVED_PER_INVOCATION && !fluidCellsToProcess.isEmpty()) {
            p3dSource = fluidCellsToProcess.remove(Utils.getRandomBetween(0, (fluidCellsToProcess.size() - 1) / 4)); // Random del primer 25% de tiles

            // Miramos si est� dentro del mapa
        ...
        for (Point3DShort alNewPoint : alNewPoints) {
            p3dTemp = alNewPoint;
            if (!cells[p3dTemp.x][p3dTemp.y][p3dTemp.z].isFluidCheckList() && cells[p3dTemp.x][p3dTemp.y][p3dTemp.z].getTerrain().hasFluids()) {
                ...
                fluidCellsToProcess.add(alNewPoint);
    ...
    /**
     * Evapora fluidos con fuerza 1 y sin fluidos con m�s fuerza a los lados Evapora el 12.5% primero de fluids
     */
    private void evaporateFluids() {
        Cell cell;
        Point3DShort p3ds;
        if (!fluidEvaporation.isEmpty()) {
            ...
            while (iNumFluidsToEvaporate > 0 && !fluidEvaporation.isEmpty()) {
                ...
                if (cell.getTerrain().getFluidCount() == 1) {
                    // Miramos que no haya fluidos con m�s fuerza a los lados
        ...
        if (p3d.z < (World.MAP_DEPTH - 1)) {
            if (cell.isDigged()) {
                if (checkFluidMovementCells(cell, p3d.x, p3d.y, p3d.z + 1) > 0) {
                    // Todo ok, comprobamos los fluid elevator, s�lo en este caso (fluidos de arriba a abajo)
    ...
    private short checkFluidMovementCells(Cell cellSource, int x, int y, int z, boolean bElevatorOnSource) {
        ...
        if (item != null) {
            ...
            if (imi != null) {
                if (imi.isBlockFluids()) {
                    ...
                    if (imi.isDoor()) {
                        // Es puerta, miramos si est� cerrada o bloqueada
        ...
        if (iSourceCount > 1 && iSourceCount > (iDestinationCount + 1)) {
            ...
            // Si llega aqu� es que no se puede mover (por lo menos a una casilla con <2 de fuerza)
            // Vamos a mirar si se puede mover a una casilla con <1 fuerza
            // S�lo podr� si tiene una casilla vecina con >1 fuerza
    ...
    public void removeItemToBeHauledByItemID(int iItemID) {
        for (int i = 0; i < itemsToBeHauled.size(); i++) {
            if (itemsToBeHauled.get(i) == iItemID) {
    ...
    public void removeItemToBeHauledByPosition(int iPosition) {
        if (!itemsToBeHauled.isEmpty() && iPosition < itemsToBeHauled.size()) {
            itemsToBeHauled.remove(iPosition);
        }
    }

    /**
     * Limpia todos los datos (se usa cuando se sale de la partida y se va al men� principal)
     */
    public void clear() {
        cells = null;
        citizenIDs = new ArrayList<>();
        soldierIDs = new ArrayList<>();
        citizenGroups = new CitizenGroups();
        soldierGroups = new SoldierGroups();

        heroIDs = new ArrayList<>(2);
        exploringHotPoints = new ArrayList<>();
        maxHeroXP = 0;
        oldHeroes = new ArrayList<>(2);
        oldHeroesDied = new ArrayList<>(2);
        currentCaravanData = null;
        enemiesKilled = null;
        projectiles = null;

        livingsDiscovered = new HashMap<>();
        livingsUndiscovered = new HashMap<>();
        LivingEntity.clear();
    ...
    /**
     * Devuelve un hotpoint de exploraci�n para un h�roe
     *
     * @param iASZID
     * @return
     */
    ...
    public void readExternal(ObjectInput in) throws IOException, ClassNotFoundException {
        ...
        if (Game.SAVEGAME_LOADING_VERSION >= Game.SAVEGAME_V12) {
            ...
            // Comentado pq el n�mero de levels a�n no est� seteado, despu�s de cargar el mundo los seteo
