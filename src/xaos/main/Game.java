...
public final class Game {
     ...
     *
     * @param campaignID ID de la campa�a, null en caso de partida normal
     * @param missionID  ID de la misi�n, null en caso de partida normal
     */
    ...
    private static void setupGame(boolean bNewGame, String sCampaignID, String sMissionID, boolean bForzedZip) {
        ...
        // Esto es un parche para solventar un bug. Antes no se borraban los ciudadanos de los job grupos al morir, y podr�a estar corrupto
        ...
        // Paramos la m�sica del main men� y arrancamos la m�sica in-game
...
//					MessagesPanel.addMessage (MessagesPanel.TYPE_SYSTEM, Messages.getString ("Game.5"), ColorGL.GREEN); //$NON-NLS-1$
    ...
    public static void generateBury(Cell[][][] cells) {
        ...
        while (bContinue) { // Nada de burying en el �ltimo nivel
            ...
            if (bd.getHash() != null) {
                ...
                if (iMaxZ == -1) {
                    ...
                    // Comprobaci�n pq no estoy seguro que se pueda salir de rango
    ...
    /**
     * Obtiene eventos del mouse y llama al "panel" correspondiente seg�n donde sea el click
     */
    private void checkMouseEvents() {
        ...
        while (Mouse.next()) {
            ...
            if (Mouse.getEventButtonState()) {
                ...
                if (mouseButton == 0) {
                    // Bot�n izquierdo pulsado, miramos a qu� panel pertenece y llamamos a su funci�n mousePressed (x, y) (con x e y relativas al panel)
                    // Primero miramos que no haya un contextmenu
                    if (getCurrentState() == STATE_SHOWING_CONTEXT_MENU) {
                        // Context men�, miramos donde clica
                    ...
                    if (UIPanel.typingPanel != null) {
                        ...
                        // Bot�n derecho pulsado, cancelamos tarea (si hay)
        ...
        if (isMouseScrollON() && Mouse.isInsideWindow()) {
            if (!getPanelMainMenu().isActive()) {
                final int BORDE = UIPanel.PIXELS_TO_BORDER; // Si se acerca X pixels al borde moveremos la c�mara
                if (mouseX < BORDE) {
                    ...
                    // Miramos que no est� cerca del bot�n de abrir/cerrar el menu
            ...
            if (Keyboard.getEventKeyState()) {
                ...
                if (getPanelMainMenu().isActive()) {
                    ...
                    if (iFN == UtilsKeyboard.FN_TOGGLE_FULLSCREEN) {
                        ...
                        // Ha tecleado algo en el main menu, quiz� seteando el nombre del savegame
    ...
    public void run() {
        ...
        // Hacemos sonar la m�sica del main men�
        UtilsAL.play(UtilsAL.SOURCE_MUSIC_MAINMENU);
//		int checkTrigger = (4 * FPS_INGAME); // Para que lo mire nada m�s empezar, en el primer ciclo
...
*/
            ...
            // }
            // fps++;
            // Pongo este m�todo pq dice la doc de LWJGL que actualiza los timers y tiene que llamarse lo m�s posible
