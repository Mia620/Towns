...
public class Building extends Entity implements Externalizable {
    ...
    /**
     * Indica si ya est� operativo
     */
    private boolean operative;

    /**
     * Son los prerequisitos para construirse, a medida que los aldeanos los
     * traigan ir� desapareciendo Cuando lleguen a 0 el edificio pasa a estar
     * operativo
     */
    private ArrayList<int[]> prerequisites;
    private ArrayList<int[]> prerequisitesLiving;

    private boolean nonStop; // Indica si siempre est� produciendo items o se hace manual
    private ArrayList<Item> itemQueue; // Es la lista de items en cola para producirse
    private String lastItem; // Indica el �ltimo item producido, es �til cuando est� en modo non-stop
    private int counter; // Se usa para contar el tiempo de los items autom�ticos
     ...
     *
     * @param item Objeto con las caracter�sticas del edificio
    ...
    /**
     * Fills a contextual men� refering buildings of a cell
     *
     * @param cell
     * @param sm
     */
        ...
        if (cell.hasBuilding()) {
            ...
            if (building != null) {
                ...
                if (building.isOperative() && !bmi.isAutomatic()) {
                    ...
                    // Si tiene cosas en cola a�adimos el men� de quitarlas
    ...
    public void delete() {
        ...
        if (iIndex != -1) {
            ...
            BuildingManagerItem bmi = BuildingManager.getItem(getIniHeader());
            // Quitamos tambi�n las marcas de la/s celda/s (de edificio y de A*ZoneID)
    ...
    public boolean nextTurn() {
        ...
        if (getLastItem() == null) { // En caso de edificio autom�tico el primer item se pone al crearlo (Task.java)
            return false;
        }

        // Tenemos last item, y est� en non-stop, le metemos otro
        BuildingManagerItem bmi = BuildingManager.getItem(getIniHeader());
