...
public class Container implements Externalizable {
    ...
    public static SmartMenu createContainerMenu(int iContainerID) {
        ...
        // A�adimos el enable todo/disable todo
        ...
        // A�adimos 1 linea por cada objeto del tipo de container (para poner/quitar)
    ...
    /**
     * Se recorre todo el men� y setea el valor de los items a enabled o disabled seg�n la configuraci�n del contenedor
     *
     * @param iContainerID
     * @param menuPile
            ...
            if (smAux.getType() == SmartMenu.TYPE_MENU) {
                ...
                if (smAux.getCommand().equals(CommandPanel.COMMAND_CONTAINER_ENABLE_ITEM) || smAux.getCommand().equals(CommandPanel.COMMAND_CONTAINER_DISABLE_ITEM)) {
                    ...
                    if (container.getType().contains(smAux.getParameter2())) {
                        // Existe, pu�s lo ponemos para disable
     ...
     *
     * @return true si el container a�n existe en el mundo
     */
    public boolean refreshTransients() {
        ...
        if (!itemOnWorld) {
            // Item no est� en el mundo, miramos el carrying de los citizens
            Citizen citizen;
    ...
    /**
     * A�ade un item al container si o si, no chequea si cabe ni si es otro container ni nada
     *
     * @param item
     */
    ...
    /**
     * Indica si hay items que no deben ir ah�
     *
     * @return
     */
    ...
    public Item removeItem(int iID) {
        for (int i = 0; i < itemsInside.size(); i++) {
            if (itemsInside.get(i).getID() == iID) {
                ...
                Item.removeItem(item); // Para que reste 1 al n�mero de items, ya que el item no existe realmente
                World.setTownValue(World.getTownValue() - PricesManager.getPrice(item));
    ...
    /**
     * Elimina un item del container y lo devuelve bas�ndose en una lista de iniheaders
     *
     * @param iID ID del item a devolver
     * @return item del container o null si no lo encuentra
     */
    ...
    /**
     * Elimina un item del container y lo devuelve bas�ndose en un iniheader
     *
     * @param iID ID del item a devolver
     * @return item del container o null si no lo encuentra
     */
    ...
    /**
     * Elimina un item de comida del container y lo devuelve. Pillamos el de m�s value
     *
     * @param iID ID del item a devolver
     * @return item del container o null si no lo encuentra
     */
    ...
    public boolean itemAllowed(Item item) {
        ...
        // Si llega aqu� es que no es container y es stackable, miramos si cabe
