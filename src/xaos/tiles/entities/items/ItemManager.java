...
public class ItemManager {
            ...
            while (itItems.hasNext()) {
                ...
                if (imi.getMaxAgeTerrain() != null && !imi.getMaxAgeTerrain().isEmpty()) {
                    if (TerrainManager.getItem(imi.getMaxAgeTerrain()) == null) {
                        Log.error(Messages.getString("ItemManager.2") + " [" + imi.getMaxAgeTerrain() + "]", "ItemManager"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
                        Game.exit();
                    }
                }
    ...
    /**
     * Devuelve un item a random a partir de un nivel m�nimo y m�ximo o null si
    ...
    private static void loadXMLItems(String sXMLName, boolean bLoadingMain) {
        try {
            ...
            // Lo recorremos entero y vamos a�adiendo los items (ItemManagerItem) a la hash
            ...
            Log.error(Messages.getString("ItemManager.6") + sXMLName + Messages.getString("ItemManager.7") + e + "]", "ItemManager"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
            Game.exit();
        }
    }
