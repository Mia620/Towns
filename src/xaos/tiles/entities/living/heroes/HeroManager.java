...
public class HeroManager {
    ...
    private static void loadXML(String sXMLName) {
        try {
                ...
                if (node.getNodeType() == Node.ELEMENT_NODE) {
                    ...
                    for (int j = 0; j < nodesPrerequisites.getLength(); j++) {
                        ...
                        if (nodePrerequisite.getNodeType() == Node.ELEMENT_NODE) {
                            ...
                            // A�adimos la lista a la hash que toque
                            if (iType == 1) {
            ...
            Log.log(Log.LEVEL_ERROR, Messages.getString("HeroManager.0") + sXMLName + "] [" + e + "]", "HeroManager"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
            Game.exit();
        }
    }
