...
public abstract class LivingEntity extends Entity implements Externalizable {
    ...
    // Flags
    private final static byte FLAG_WAITING_FOR_PATH = 1; // 00000001 (Indica si el ser viviente est� esperando camino)
    private final static byte FLAG_GROUPING = 2; // 00000010 (Indica si llama a sus amigos cuando le atacan)
    private final static byte FLAG_FIGHTING = 4; // 00000100 (Indica si est� luchando)
    /**
    ...
    private int maxAge; // Edad m�xima del friendly

    private byte facingDirection;

    private FocusData focusData; // Guarda el ID + type de un entity al que est� atacando
    ...
    private Point2D.Float positionOffsetConstants = new Point2D.Float(0, 0); // Se usa en el dibujado, es lo que hay que a�adir al offset a cada frame
    ...
    public LivingEntity(String iniHeader) {
        ...
        LivingEntityManagerItem lemi = LivingEntityManager.getItem(getIniHeader()); // Ojo, aqu� hay que usar el getIniHeader, pq puede estar cambiado
    ...
    /**
     * A�ade el Living a la lista y suma 1 al n�mero de los mismos
     *
     * @param itemIniHeader IniHeader del living
     * @return el n�mero de livings que hay de ese tipo, incluyendo el acabado de a�adir
     */
    ...
    /**
     * Devuelve el n�mero de livings en el mundo del tipo pasado
     *
     * @param sIniHeader IniHeader del living
     * @return el n�mero de livings en el mundo del tipo pasado
     */
    ...
    /**
     * Setea el n�mero de livings para un living dado
     *
     * @param sIniHeader IniHeader del living
    ...
    /**
     * Pone en juego un item munici�n (ej: flecha, bola de fuego, ...) Tiene en cuenta las cargas del arma y controla si se acaban y se rompe
     *
     * @param attacker
     * @param victim
     */
    public static void doRangeAttack(LivingEntity attacker, LivingEntity victim, boolean bCheckAttackTurn) {
        ...
        int damage = attacker.getLivingEntityData().getDamageCurrent(); // Se pone aqu� ppq m�s abajo el arma puede estar destruida
        int maxDistance = attacker.getLivingEntityData().getLOSCurrent(); // Se pone aqu� pq m�s abajo el arma puede estar destruida
    ...
    /**
     * Indica si el personaje est� listo para caminar. Actualiza el contador de turnos
     *
     * @param living
     * @return
     */
    public static boolean canWalkNext(LivingEntity living, int livingType) {
        ...
        if (livingType == TYPE_CITIZEN) {
            // Miramos la reducci�n de velocidad debido a hambre y felicidad
            ...
            // Eventos globales, s�lo a townies
    ...
    /**
     * Indica si el personaje est� listo para lanzar otro ataque. Actualiza el contador de turnos
     *
     * @param attacker
     * @return
     */
    ...
    /**
     * Intenta dar un golpe de atacante a v�ctima. Si lo consigue resta puntos de vida a la v�ctima
     *
     * @param attacker Atacante
     * @param victim   V�ctima
     * @return true si el golpe se da con �xito
     */
     ...
     * @param attacker           Atacante
     * @param victim             V�ctima
     * @param bCheckAttackTurn   Indica si hay que comprobar si el atacante est� listo para el siguiente ataque
     * @param rangedWeapon       Indica si el ataque es ranged. En otro caso valdr� null. Se usa para obtener los verbos
     * @param iRangedDamage      Da�o fijo, se usa para los ataques ranged, ya que el da�o se calcula en el momento de lanzar el proyectil. Aqu� se aplicar� el % en base a la distancia del enemigo
     * @param iRangedMaxDistance Distancia m�xima en el momento del disparo.
    ...
    public static boolean doHit(LivingEntity attacker, LivingEntity victim, boolean bCheckAttackTurn, ItemManagerItem rangedWeapon, int iRangedDamage, int iRangedMaxDistance, boolean bRangedHit) {
        ...
        // Calculamos el porcentaje de ataque en base a la defensa de la v�ctima
        // Hay un m�nimo de 5% de atacar y un m�ximo de 95%
        int iPCT;
        ...
        if (Utils.getRandomBetween(1, 100) <= iPCT) {
            // Hit
            // Si es ranged miramos la dist�ncia
            int iDanyo;
            if (rangedWeapon != null) {
                // Calculamos el % de da�o que har� seg�n la distancia (de 100% a 20%)
            ...
            // Antes de restar el da�o miramos que no estubiera ya muerto (para el medallero)
            ...
            // Notificamos a la v�ctima
            victim.hitted(attacker, true, iDanyo);

            // Si el atacante es h�roe o soldado le sumamos experiencia
            // Si es h�roe o citizen sumamos 1 a los enemigos de ese tipo matados
            ...
            // La v�ctima ha muerto de este golpe, sumamos 1 al medallero en caso de HERO o CITIZEN
            ...
            // Si el atacante tiene un effect con attackDOT, le metemos un efecto de DOT a la v�ctima
    ...
    /**
     * Indica si hay alg�n enemigo de la livingentity pasada en LOS del punto pasado, en ese caso devuelve el FocusData del mismo
     *
    ...
    /**
     * Indica si hay alg�n enemigo en LOS de la livingentity pasada, en ese caso devuelve el FocusData del mismo
     *
    ...
    private static FocusData hasEnemyInLOSAtCell(String sLivingSource, short sourceX, short sourceY, short sourceZ, short pointX, short pointY, boolean bAttackAllies) {
        ...
        if (World.getCell(sourceX, sourceY, sourceZ).getAstarZoneID() == World.getCell(pointX, pointY, sourceZ).getAstarZoneID()) {
            ...
            // Miramos que haya vista "directa", teniendo en cuenta muros y dem�s panochadas
    ...
    /**
     * Devuelve una lista de livings en uso por aldeanos que NO sea el aldeano pasado como par�metro
     *
     ...
     *
     * @param p3dIni           Punto inicial de b�squeda
     * @param alHeaders        Lista de numeric Iniheaders de los livings
     * @param near             Indica si hay que buscar la m�s cercana o cualquiera
    ...
    public static LivingEntity searchLiving(Point3DShort p3dIni, int[] alHeaders, boolean near, ArrayList<Integer> alLivingsToAvoid) {
        // CUIDADO, M�TODO DUPLICADO ABAJO, SI SE MODIFICA HAY QUE MODIFICAR EL OTRO
                ...
                if (UtilsIniHeaders.contains(alHeaders, le.getNumericIniHeader())) {
                    if (alLivingsToAvoid == null || !alLivingsToAvoid.contains(Integer.valueOf(le.getID()))) {
                        if (!near) {
                            ...
                            // Usaremos el iNum (n�mero de livings en el mundo) para saber si hemos acabado (performance trick)
                            iNum--;
                            if (iNum == 0) {
                                break;
                            }
    ...
    public static LivingEntity searchLivingForcedASZID(Point3DShort p3dIni, int[] alHeaders, boolean near, ArrayList<Integer> alLivingsToAvoid, int iASZID) {
        // CUIDADO, M�TODO DUPLICADO ARRIBA, SI SE MODIFICA HAY QUE MODIFICAR EL OTRO
    ...
    public static boolean isCellAllowed(Cell cell) {
        ...
        if (cell.hasBuilding()) {
            ...
            if (groundDataChar == Building.GROUND_TRANSITABLE || groundDataChar == Building.GROUND_ENTRANCE) {
                ...
                // Celda NO principal, s�lo se puede pasar si el edificio no est� a�n operativo
    ...
    /**
     * Fills a contextual men� refering a living entity of a cell
     *
     * @param cell
     * @param sm
     */
    ...
    /**
     * Devuelve una living entity hate de la celda y living pasada Se le pasa un focus, si tiene alguien ah�, eso tiene preferencia
     *
     * @param sIniHeader
     * @param p3d
     * @return
    ...
    /**
     * Analiza la cadena de offset (que es "x, y") y devuelve el valor num�rico de una de las 2
     *
    ...
    public void setCoordinates(short iX, short iY, short iZ) {
        int iIndex = -1;
        if (getX() != -1) { // Con esto miro que no sea una living nueva (que no tendr�a celda source)
    ...
    /**
     * Setea la constante, tambi�n cambia el facing direction
     */
    protected void updatePathConstantOffsets() {
        ...
        if (getPath().size() == 0) {
            ...
            // Calculamos las constants seg�n hacia donde vaya
            ...
            // Si est� cambiando de nivel hay que sumar, restar a la Y
    ...
    public void updateFacingDirection(int x1, int y1, int z1, int x2, int y2, int z2) {
        byte facingReturn;
        // if (z1 != z2) {
        // // Sube/baja, no cambiamos la direcci�n
        // facingReturn = getFacingDirection ();
        // } else {
        // Miramos las 8 direcciones
        if (x1 == x2) {
            // S�lo se ha movido en el plano Y
            if (y1 < y2) {
            ...
            if (y1 == y2) {
                // S�lo se ha movido en el plano X
                if (x1 < x2) {
                ...
                // Si llega aqu� es que es una diagonal si o si
                if (x1 < x2) {
                    if (y1 < y2) {
    ...
    /**
     * Mueve a la living SI o SI (si puede, claro) Se usa cuando est� encima de un muro (ocurre cuando se acaba de construir), encima de fluidos o en encima de un hole
     *
     * @return true si ha podido, false si no puede (en ese caso deber�a palmar o algo)
     */
    public boolean forceMove() {
        ...
        // Si la casilla de arriba est� minada, miramos las casillas diagonal/parriba
        ...
        // Si llega aqu� es que no hay ni un punto bueno
        // En caso de hole (digged) caer� abajo, en otro caso palmar�
    ...
    public void setDestination(int x, int y, int z, boolean bRemoveTaskIfFails, int iNumCellsToRemove) {
        ...
        // Si el destino no es allowed lo sacamos de la tarea (si tiene) (s�lo en caso de aldeanos)
        int iLivingType = LivingEntityManager.getItem(getIniHeader()).getType();
        ...
        // Buscamos camino
        // Bresenham para empezar (linea recta) (si est� en el mismo nivel)
        // if (getZ () == z) {
        // setPath (Utils.bresenhamLine (getX (), getY (), x, y, z, livingType));
        // }
        // Camino vac�o, eso es que en linea recta topa con algo (o est� en distintos niveles), usamos A* para buscar la ruta
        // if (getPath ().isEmpty ()) {
    ...
    public void delete(boolean dead) {
        ...
        if (dead) {
            if (lemi.getType() == TYPE_HERO) {
                // Caso h�roes, si se han pirado del mapa no hay que sacar drop
                Hero hero = (Hero) this;
        ...
        if (dead) {
                ...
                if (enemy.getCarryingData() != null) {
                    ...
                    if (enemy.getCarryingData().getCarrying() != null) {
                        ...
                        if (p3dDrop != null) {
                            ...
                            // Si no puede soltarlo nos lo trincamos, s�lo hay que mirar containers para que se limpien bien las listas
                            if (imi.isContainer()) {
                                Game.getWorld().deleteContainer(itemCarrying.getID());
                            }
                        }
            ...
            // No lo matamos, eso es que lo ha pillado alg�n aldeano, le ponemos la direction pertinente para que quede bien
            // Tambi�n podr�a ser un hero que se pira (o un enemy robbery, en los 2 casos borramos containers si llevaban alguno)
        ...
        if (lemi.getType() == TYPE_CITIZEN) {
            ...
            if (iIndex != -1) {
                // Lo sacamos de la lista de h�roes
            ...
            if (bHeroLeaved) {
                // Lo metemos tambi�n en la lista de h�roes que se han pirado (para que quiz� vuelvan en el futuro)
                Game.getWorld().getOldHeroes().add(this);
            } else {
                // H�roe muerto, si tiene nombre especial (caso Sips, punchwood, ...) lo metemos en la lista de muertos para que no aparezca de nuevo
    ...
    protected void doFocus(LivingEntityManagerItem lemi) {
        // Si el "enemigo" (<hate>) a�n existe vamos hacia �l
        ...
        if (leHate != null) {
            ...
            if (bRanged) {
                if (Math.abs(leHate.getCoordinates().x - getX()) <= getLivingEntityData().getLOSCurrent() && Math.abs(leHate.getCoordinates().y - getY()) <= getLivingEntityData().getLOSCurrent()) {
                    // Est� a rango, disparamos
                    doRangeAttack(this, leHate, true);
                    setPath(null); // Importante
                } else {
                    // No est� a rango, nos olvidamos, pero nos acercamos si podemos
            ...
            if (lemi.getType() == TYPE_CITIZEN) {
                // Citizen, le quitamos el sue�o en caso de lucha
    ...
    public boolean nextTurn() {
        ...
        for (int i = (getLivingEntityData().getEffects().size() - 1); i >= 0; i--) {
            ...
            if (effectData.getLasts() <= 0) {
                ...
                if (emi.getLasts() != null) { // Tiene lasts, pues puede acabar
                    ...
                    // No tiene lasts, reseteo para que no se pase de rango (aunque podr�an pasar a�os reales para que esto pase, creo)
        ...
        // Food needed (nunca valdr� 0, si vale 0 es que el bicho no necesita comer)
        if (getFoodNeededTurns() != 0) {
            ...
            if (getFoodNeededTurns() == 0) {
                ...
                // Est� en negativo, miramos si ya toca palmar
        ...
        if (!isCellAllowed(x, y, z)) {
            boolean palma = true;
            // En caso de muro (o puerta bloqueada) o agua o hole no palma, son as� de guais
            Cell cell = World.getCell(x, y, z);
            if (cell.getTerrain().hasFluids()) {
                palma = false;
            ...
            if (cell.isDigged()) { // Cuidado con el �rden, la comprobaci�n de digged debe ir aqu� ya que sabemos que no hay item en la celda
                palma = false;
                bWallFluidsHole = true;
            }
        ...
        if (lemi.getType() == TYPE_CITIZEN) {
            ...
            // Eat sleep + turnos m�nimos de stay (HEROS)
            ((Hero) this).updateCounters();
        }

        // Si est� en un muro o fluidos lo movemos si o si
        if (bWallFluidsHole) {
            if (!forceMove()) {
                if (lemi.getType() == TYPE_CITIZEN || lemi.getType() == TYPE_HERO) {
                    MessagesPanel.addMessage(MessagesPanel.TYPE_ANNOUNCEMENT, getCellDeadMessage(x, y, z, lemi.getType()), ColorGL.RED, getCoordinates());
        ...
        // Discover in LOS
        if (lemi.getType() == TYPE_CITIZEN || lemi.getType() == TYPE_HERO) {
            // �sto es para que descubran celdas NO discovered
            if (getCheckLOSCounter() == 0) {
                if (getCoordinates().z > 0) { // Las celdas de arriba est�n todas discovered
        ...
        if (getFocusData().getEntityID() == -1 && fleeing == 0) {
            ...
            if (getCheckLOSCounter() == 0) {
                ...
                if (fd != null && fd.getEntityID() != -1) {
                    ...
                    // No ve a nadie en LOS, miramos si hay alg�n item para pillar (caso heroes)
        ...
        if (getPath().isEmpty()) {
            ...
            if (getFocusData().getEntityID() != -1) {
                ...
                // No hay nadie en linea de visi�n
                ...
                if (lemi.getFollowEntity() != null && lemi.getFollowEntity().length > 0) {
                    ...
                    if (getLivingEntityData().getFollowTurnsCounter() <= 0) {
                        ...
                        // Buscamos una living de estas (normalmente s�lo habr� 1 en el mundo, caso Sips) y le hacemos un setdestination
                        ...
                        if (iTrys > 0) {
                            // Para hacer que vaya a items a random, pillar� unas coordenadas a random y buscaremos la living m�s cercana
                ...
                if (lemi.getType() == TYPE_ENEMY) {
                    ...
                    if (lemi.getCaravan() != null && Game.getWorld().getCurrentCaravanData() != null) {
                        // Caravana sin path, si a�n tiene el estado de coming hay que updatearlo
            ...
            // Obtenemos el primer punto, NO se elimina a�n del path
            Point3DShort point = getPath().get(0);

            if (point == null || (point.x == x && point.y == y && point.z == z)) {
                // Hemos llegado al final del camino, path vac�o
                clearPath();
            } else {
                ...
                if (leHate != null) {
                    ...
                    // Miramos que no haya un wallConector (puerta), en ese caso le metemos hostias hasta que pete (s�lo si no est� abierta y es enemy)
                    // En caso de friendly no puede pasar si est� cerrada o locked (excepto caravanas)
                    ...
                    if (!bWallConector) {
                        ...
                        if (lemi.getType() != TYPE_CITIZEN || (((Citizen) this).getCurrentTask() == null || ((Citizen) this).getCurrentTask().getTask() != Task.TASK_HEAL)) {
                            ...
                            // Citizen heleandose (cur�ndose, si)
                            bCamina = true;
                        }
                        ...
                        if (bCamina) {
                            if (canWalkNext(this, lemi.getType())) {
                                point = getPath().remove(0); // En principio no har�a falta igualar otra vez el point a esto
     ...
     *
     * @param iPCT Porcentaje de moverse (suele ser menor que el par�metro, ya que intentar� ir a un sitio y si no es allowed no se mover�)
                            ...
                            if (iDirection >= 0 && iDirection < 8 && Utils.getRandomBetween(1, 2) == 1) {
                                ...
                                if (iAuxX >= 0 && iAuxX < World.MAP_WIDTH && iAuxY >= 0 && iAuxY < World.MAP_HEIGHT) {
                                    if (isCellAllowed(iAuxX, iAuxY, iAuxZ)) {
                                        ...
                                        // Un 10% m�s de moverse otra casilla
    ...
    /**
     * Este m�todo es para notificar a una living entity que ha sido golpeado, se le pasa el atacante
     *
    ...
    /**
     * A�ade un efecto a la living, si ya lo tiene reseteamos el contador (lasts)
     *
     * @param edi
     */
    ...
    public void addEffect(EffectManagerItem emi, boolean applyEffectStuff, boolean verbose) {
        ...
        // Si llega aqu� es que no es immune al efecto por via efectos
        if (iIndexSameEffect == -1) {
            // Efecto nuevo, miramos immunidades (s�lo falta mirar immunidades por living)
    ...
    public void removeEffect(String id, int iLivingType, boolean verbose) {
        ...
        for (int i = (getLivingEntityData().getEffects().size() - 1); i >= 0; i--) {
            ...
            if (effectData.getEffectID().equals(id)) {
                ...
                // Si era un effect de atacar aliados, borramos el target para que no siga atac�ndolos
                if (effectData.isAttackAllies()) {
                    getFocusData().setEntityID(-1);
                    getFocusData().setEntityType(TYPE_UNKNOWN);
                    setFighting(false);
