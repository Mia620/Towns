...
public class Cell implements Externalizable {
    ...
    // Flags
    public final static int FLAG_ORDERS = 1; // 00000001 (Indica si la celda tiene �rdenes)
    public final static int FLAG_DISCOVERED = 2; // 00000010 (Indica si la celda est� descubierta)
    ...
    public final static int FLAG_PATROL_POINT = 8; // 0000001000 (Indica si alg�n soldado tiene ese punto de patrulla)
    ...
    public final static int FLAG_DIGGED = 64; // 0001000000 (Indica si la casilla de abajo est� minada)
    public final static int FLAG_SHADOW = 128; // 0010000000 (Indica si la casilla est� en la sombra)
    public final static int FLAG_BLOCKY = 256; // 0100000000 (Indica si la casilla est� ocupada por un bloque que tapa las cosas de debajo)
    public final static int FLAG_MINED = 512; // 10 00000000 (Indica si la casilla est� minada)
    ...
    public final static int FLAG_FLUID_CHECK_LIST = 131072; // 10 00000000 00000000 (Indica si la casilla est� en la lista de fluidos a chequear
    public final static int FLAG_OPEN = 262144; // 10 00000000 00000000 (Indica si la casilla est� en la lista de fluidos a chequear
    ...
    /**
     * Es un ID para saber r�pidamente si hay camino entre 2 puntos 2 celdas con distinto ID significa que no hay camino
     */
    ...
    /**
     * Constructor alternativo, se le pasa un Terrain, deja la entity vac�a
     */
    ...
    /**
     * Dada una casilla actualiza su A*zoneID seg�n las adyacentes
     *
     * @param p3d
     */
    ...
    /**
     * Comprueba que el array de IDs contenga el pasado- Usa la variable global neighborIDIndex para saber el tama�o
     *
     * @param iID
     * @return
     */
     ...
     * @param y     Y
     * @param z     Z
     * @param bFull Indica si se est� haciendo un merge de todo, en ese caso el getNeighbour no hace falta que mire todos los vecinos
     */
    public static void mergeZoneID(int x, int y, int z, boolean bFull) {
        // Actualizamos el ID de zona A* seg�n las casillas adyacentes
    ...
    public static void generatePaintUnder(Cell[][][] cells) {
        ...
        for (int x = 0; x < World.MAP_WIDTH; x++) {
            for (int y = 0; y < World.MAP_HEIGHT; y++) {
                for (int z = 0; z < (World.MAP_DEPTH - 1); z++) { // La �ltima layer es false
    ...
    public static void generateLightsItemRemovedCellMined(int x, int y, int z, int iRadius) {
        ...
        // Ahora, teniendo en cuenta el radio m�ximo de todos los objetos que producen luz, miramos las celdas a iluminar
    ...
    public static void generateLights(Cell[][][] cells, int x, int y, int z, boolean bProjectiles) {
        ...
        if (imi != null) {
            if (imi.getLightRadius() != 0) {
                ...
                // Todas las casillas de las lineas (y las que est�n por arriba y abajo, teniendo en cuenta el radio) tendr�n luz, hasta que lleguen a un non-mined o wall
                int iX, iY;
                // Norte del cuadrado
                iY = y - iRadius;
                if (iY < 0) {
    ...
    public static void generateFullShadows(int x, int y, int z) {
        ...
        while (x < (World.MAP_WIDTH) && z < (World.MAP_DEPTH)) {
            if (bPutShadowOnNext) {
                ...
                Cell cell = cells[x][y][z];
                // Aqu� s�lo entra en las primeras iteraciones
...
//                        Game.getWorld().addFluidCellToProcess(x + i, y + j, z + k, false);
    ...
    public void setEntity(Entity entity) {
        ...
        if (entity == null || (this.entity != null)) {
            ...
            if (this.entity != null && this.entity instanceof Item cellItem) {
                ...
                // Si era un container, restamos tambi�n el precio de lo de dentro
                if (imi.isContainer()) {
                ...
                // Si la casilla de abajo no est� discovered la ponemos discovered ahora
        ...
        // Miramos si el item puede ir aqu�, en caso contrario lo meteremos en la lista de objetos para haulear
        if (entity != null && entity instanceof Item) {
            if (stockpile != null) {
                ...
                // No hay pila, lo metemos en la lista de haul, el taskmanager ya mirar� si es un item haul o no
                Game.getWorld().addItemToBeHauled((Item) entity);
            }
        }
        ...
        if (entity != null && entity instanceof Item) {
            ...
            // Lo a�adimos a la lista de check para fall
        ...
        if (iLightRadiusItemRemoved >= 0) {
            // Se va a borrar un item que produc�a luz, regeneramos
            generateLightsItemRemovedCellMined(coord.x, coord.y, coord.z, iLightRadiusItemRemoved);
            // Si el nuevo item tiene light el m�todo generateLightsItemRemoved ya lo habr� checkeado
        } else {
            // Light new item
            if (imi != null && imi.getLightRadius() > 0) {
                generateLights(World.getCells(), coord.x, coord.y, coord.z);
            } else {
                // Nuevo item no produce luz, pero quiz� es un lightblocker
    ...
    /**
     * Indica si la celda contiene una living espec�fica por tipo, la devuelve si la encuentra (la primera que pilla)
     *
     * @param type
     * @return
     */
    ...
    public void setStockPileID(int iSPID) {
        ...
        if (stockPileID == 0) {
            // Ha borrado la pile de esta celda, si hab�a un item lo metemos en la lista de hauling
    ...
    public void fallThings() {
        if (isDigged()) {
            // Miramos la casilla de abajo (en teor�a no hace falta pq si o si estar� mined)
            // Buildings
