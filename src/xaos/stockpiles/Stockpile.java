...
public class Stockpile implements Externalizable {
    ...
    /**
     * Inhabilita todos los elementos de la pila. Si se le pasa un subtype deshabilita s�lo esos
     *
     * @param iID
     */
    ...
    /**
     * Se recorre todo el men� y setea el valor de los items a enabled o disabled seg�n la configuraci�n de la pila
     *
     * @param iStockpileID
     * @param menuPile
    ...
    public static SmartMenu createPilePanelMenu(int iStockpileID) {
        ...
        // A�adimos 1 linea por cada objeto del tipo de stockpile (para poner/quitar)
        ...
        for (int i = 0; i < alElementsWithSubtypeName.size(); i++) {
            ...
            // A�adimos el enable todo/disable todo (del subtipo)
    ...
    private void setPoints(ArrayList<Point3DShort> points) {
        ...
        for (Point3DShort point : points) {
            if (World.getCell(point).getEntity() != null) {
                setFilledPoints(getFilledPoints() + 1);
            }

            Game.getWorld().addItemToBeHauled(World.getCell(point).getItem());
        }
    }
    ...
    /**
     * Indica si el item pasado (iniheader) est� permitido por la stockpiles
     *
     * @param sItemIniHeader
     * @return true si el item pasado (iniheader) est� permitido por la stockpiles
     */
