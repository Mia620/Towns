...
public class MapGenerator extends Generator {
    ...
    public static Cell[][][] generateMap(String sCampaignID, String sMissionID) {
        ...
        if (TownsProperties.DEBUG_MODE) {
            ...
            Log.debug(sLog, "MapGenerator"); //$NON-NLS-1$
        }
        ...
        if (item == null || !item.getName().equalsIgnoreCase(MapGeneratorItem.ITEM_INIT)) {
            Log.error(Messages.getString("MapGenerator.2"), "MapGenerator"); //$NON-NLS-1$ //$NON-NLS-2$
            Game.exit();
        }
        ...
        if (iNumLevelsUnderground <= 0) {
            Log.error(Messages.getString("MapGenerator.8"), "MapGenerator"); //$NON-NLS-1$ //$NON-NLS-2$
            Game.exit();
        }
        if (iNumLevelsOutside <= 0) {
            Log.error(Messages.getString("MapGenerator.11"), "MapGenerator"); //$NON-NLS-1$ //$NON-NLS-2$
            Game.exit();
        }
        if (MAINTERRAIN == null || MAINTERRAIN.trim().isEmpty()) {
            Log.error(Messages.getString("MapGenerator.4"), "MapGenerator"); //$NON-NLS-1$ //$NON-NLS-2$
            Game.exit();
        }
        if (NUM_CITIZENS < 1) {
            Log.error(Messages.getString("MapGenerator.12") + NUM_CITIZENS + "]", "MapGenerator"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
            Game.exit();
        }
        ...
        if (STARTING_EVENTS != null && !STARTING_EVENTS.isEmpty()) {
            ...
            if (alStartingEventsPCT == null || alStartingEventsPCT.size() != STARTING_EVENTS.size()) {
                Log.error(Messages.getString("MapGenerator.14"), "MapGenerator"); //$NON-NLS-1$ //$NON-NLS-2$
                Game.exit();
            }
            ...
            for (String startingEvent : STARTING_EVENTS) {
                if (EventManager.getItem(startingEvent) == null) {
                    Log.error(Messages.getString("MapGenerator.15") + " [" + startingEvent + "]", "MapGenerator"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
                    Game.exit();
                }
            }
        ...
        if (World.MAP_DEPTH > World.MAX_DEPTH) {
            Log.error(Messages.getString("MapGenerator.9") + World.MAX_DEPTH + "]", "MapGenerator"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
            Game.exit();
        }
        ...
        if (STARTING_LEVEL < 0 || STARTING_LEVEL >= World.MAP_DEPTH) {
            Log.error(Messages.getString("MapGenerator.16") + STARTING_LEVEL + "]", "MapGenerator"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
            Game.exit();
        }
        ...
        if (alBaseTerrains.size() <= 0) {
            Log.error(Messages.getString("MapGenerator.6") + MAINTERRAIN + "]", "MapGenerator"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
            Game.exit();
        }
        ...
        for (int i = 0; i < generator.getList().size(); i++) {
            ...
            if (item.getName() == MapGeneratorItem.ITEM_SEED) {
                ...
                Log.debug(Messages.getString("MapGenerator.0") + item.getName() + Messages.getString("MapGenerator.1") + XML_FILE + "]", "MapGenerator"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
            }
        }
        ...
        // Mapa generado, vamos a eliminar las celdas AIR del �ltimo nivel
        ...
        for (int z = 0; z < World.MAP_DEPTH; z++) {
            for (int x = 0; x < World.MAP_WIDTH; x++) {
                for (int y = 0; y < World.MAP_HEIGHT; y++) {
                    ...
                    if (tmi == null) {
                        Log.error(Messages.getString("MapGenerator.5") + asMap[x][y][z].getTerrainID() + "]", "MapGenerator"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
                        Game.exit();
                    }
                    if (!asMap[x][y][z].hasSpecial()) {
                        ...
                        if (asMap[x][y][z].hasSpecial()) {
                            if (asMap[x][y][z].getSpecial() == MapGeneratorItem.SPECIAL_INT_WATER_1) {
                                ...
                                Log.error(Messages.getString("MapGenerator.10") + asMap[x][y][z].getSpecial() + "]", "MapGenerator"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
                            }

                            // Flag fluid_1 de la celda
                            // No hace falta pq se chequean todas al acabar la generaci�n
        ...
        if (TownsProperties.DEBUG_MODE) {
            Log.debug(Messages.getString("MapGenerator.51"), "MapGenerator"); //$NON-NLS-1$ //$NON-NLS-2$
        }
    ...
    private static boolean airAround(Cell[][][] cells, int x, int y, int z) {
        // Miramos si alg�n vecino tiene AIR, incluido arriba y abajo
    ...
    private static void generateHeightSeed(MapGeneratorItem[][][] asMap, GeneratorItem item) {
        ...
        while (bHaySeeds) {
            // Copiamos el auxiliar (y de paso miramos que a�n haya seeds)
            bHaySeeds = false;
            for (int x = 0; x < World.MAP_WIDTH; x++) {
                for (int y = 0; y < World.MAP_HEIGHT; y++) {
                    ...
                    if (abMap[x][y]) {
                        ...
                        for (int z = 0; z <= World.MAP_NUM_LEVELS_OUTSIDE; z++) {
                            if (asMap[x][y][z].getTerrainID() == TerrainManagerItem.TERRAIN_AIR_ID) {
                                ...
                                // Tenemos un no-air
                                if (bHayAIRs) {
                                    // Tiene espacio por encima, as� que simplemente cambiamos el terrain por el de abajo
