...
public class DungeonGenerator {
    ...
    public static void generateDungeons(String sCampaignID, String sMissionID) {
        ...
        for (DungeonData alDungeon : alDungeons) {
            ...
            if (dungeonData.getType().equalsIgnoreCase(DungeonData.TYPE_CAVE)) {
                ...
                Log.error(Messages.getString("DungeonGenerator.1") + dungeonData.getType() + "]", "DungeonGenerator"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
            }
        }
    }
    ...
    private static void generateDungeonRooms(Cell[][][] cells, int iLevel) {
        ...
        // Metemos la primera room de 5x5 en un sitio sin l�quidos
        ...
        // Ahora vamos generando pasillos y rooms a partir de �sta
    ...
    /**
     * Genera pasillos y habitaciones a partir de una habitaci�n
     *
     * @param cells
     * @param cellsCopia
     * @param roomX
    ...
    private static void generateDungeonRooms(Cell[][][] cells, boolean[][] cellsCopia, int roomX, int roomY, int roomWidth, int roomHeight, int iLevel) {
        // Pasillos en las 4 direcciones y despu�s room al final de cada pasillo
        // Se llamar� recursivamente hasta que no quepan m�s
        ...
        // El orden (n, s, e, o) ser� aleatorio, usamos un array de ints (1=N, 2=S, 4=E, 8=O)
        int[] orden = new int[4];
        ...
        for (int k : orden) {
            ...
            if (k == 1) {
                ...
                // No es posible
                Log.error(Messages.getString("DungeonGenerator.0") + k + "]", "DungeonGenerator"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
            }
        }
    }
     ...
     * generateDungeonRooms(....) para que meta pasillos, m�s rooms, ....
     *
     * @param cells
     * @param cellsCopia
     * @param roomX
    ...
    private static boolean placeRoom(Cell[][][] cells, boolean[][] cellsCopia, int roomX, int roomY, int roomWidth, int roomHeight, int iLevel) {
        ...
        // Habitaci�n dentro del mapa, comprobamos que ninguna casilla ya est� ocupada o tenga fluidos alrededor
        boolean bOK = true;
        room1:
        ...
        if (bOK) {
            // La habitaci�n cabe de guais, la metemos
            for (int x = roomX; x < (roomX + roomWidth); x++) {
                for (int y = roomY; y < (roomY + roomHeight); y++) {
                    cellsCopia[x][y] = true;
                }
            ...
            // Llamamos al generateDungeon (...) para que genere pasillos y m�s rooms
    ...
    /**
     * Devuelve el n�mero de vecinos discovereds Se usa en la generaci�n de
     * dungeons
     *
        ...
        for (int i = (x - 1); i <= (x + 1); i++) {
            for (int j = (y - 1); j <= (y + 1); j++) {
                if (i >= 0 && i < World.MAP_WIDTH && j >= 0 && j < World.MAP_HEIGHT && (i != x || j != y)) {
                    ...
                    // Fuera del mapa, s�lo el 20% son discovereds
