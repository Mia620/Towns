...
public final class UIPanel {
    ...
    // BOTTOM panel
    private static ArrayList<Point> bottomPanelItemsPosition; // Array de s�lo BOTTOM_PANEL_NUM_ITEMS posiciones (9) con las coordenadas de los items que caben
    ...
    public static void setTradePanelActive(boolean tradePanelActive) {
        ...
        if (UIPanel.tradePanelActive != tradePanelActive) {
            if (tradePanelActive) {
                ...
                UIPanel.tradePanelActive = tradePanelActive; // Se pone primero pq sino el Game.resume no funcionar�

                // Se desactiva el panel, quitamos la pausa si al activar el panel el juego no estaba pausado
                if (!tradePanelActivePausedBefore) {
                    // Si antes no hab�a pausa la quitamos
                    Game.resume(false);
                }
            }
    ...
    public static void setPilePanelActive(int iPileContainerID, boolean isContainer) {
        ...
        if (iPileContainerID != -1) {
            ...
            if (menuPile != null) {
                // Cambiamos el tama�o de los iconos
    ...
    private static void createProductionPanel(SmartMenu menu) {
        ...
        int iMenu;
        bucle1:
        for (int y = 0; y < PRODUCTION_PANEL_NUM_ITEMS_Y; y++) {
        ...
        // Minibot�n para abrir/cerrar el men� de producci�n
    ...
    private static void resizePilePanel(SmartMenu menuPile) {
        ...
        if (iRows <= 1) {
            // Lo hacemos peque�o por la derecha
            int iSeparationW = (PILE_PANEL_WIDTH - tileScrollUp.getTileWidth() - 4 * tileMatsPanelSubPanel[3].getTileWidth() - iMaxItemsWidth * tileBottomItem.getTileWidth()) / (iMaxItemsWidth + 1);
            int iFirstWidth = pilePanelPoint.x + tileMatsPanelSubPanel[3].getTileWidth() + iSeparationW;
    ...
    public static void createLivingsPanel(int iPanelTypeActive, int iSoldiersGroupActive, int iCitizensGroupActive) {
        ...
        if (iPanelTypeActive == LIVINGS_PANEL_TYPE_SOLDIERS || iPanelTypeActive == LIVINGS_PANEL_TYPE_CITIZENS) {
            ...
            // Primer icono del subpanel y la separaci�n
    ...
    private void loadMenus(String sCampaignID, String sMissionID) {
        ...
        // Vamos a setear los tama�os de los iconos de los men�s para que sea proporcional al bot�n de men�
    ...
    public void initialize(String sCampaignID, String sMissionID, boolean bLoadMenus) {
        ...
        // Calculamos la posici�n de los minipaneles de scroll
        ...
        // Minibot�n para abrir/cerrar el panel de abajo
    ...
    private int renderBottomMenuPanel(int mouseX, int mouseY, int mousePanel, int iCurrentTexture) {
        ...
        if (bottomSubPanelMenu != null) {
            ...
            int iMenu;
            bucle1:
            for (int y = 0; y < BOTTOM_SUBPANEL_NUM_ITEMS_Y; y++) {
    ...
    private void renderTradePanel(int mouseX, int mouseY, int mousePanel) {
        ...
        if (caravanData == null || caravanData.getStatus() == CaravanData.STATUS_NONE) {
            ...
            Log.error("Caravan status [" + caravanData.getStatus() + "]", "UIPanel"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
            sText = null;
        }
        ...
        // Si llega aqu� es que la caravana est� lista para tradear (o est� tradeando)
    ...
    private void renderLivingsPanel(int mouseX, int mouseY, int mousePanel) {
        ...
        // Num livigs > 0, comprobamos �ndices
    ...
    /**
     * Cierra los men�s que no est�n locked indicados
     *
     * @param bottom
     * @param right
     * @param production
    ...
    /**
     * Indica si el rat�n est� en alg�n panel. Retorna un c�digo seg�n el panel
     *
     * @param x
     * @param y
     * @param doEdgeMenusStuff. Setea el delay a 0 si el mouse est� en uno de los paneles laterales, tambi�n abre/cierra menus y tal
     * @return
     */
    public int isMouseOnAPanel(int x, int y, boolean doEdgeMenusStuff) {
        /*
         * TYPING PANEL (Si est� activo ya no miraremos nada m�s)
         */
        if (typingPanel != null) {
        ...
        if (imagesPanel != null && ImagesPanel.isVisible()) {
            ...
            // Miramos tambi�n el bot�n (para hacer toggle)
...
//			return MOUSE_NONE;
        ...
        if (isBottomMenuPanelActive()) {
            ...
            if (isMouseOnBottomPanel(x, y)) { // Este check tiene que ir detr�s de los items, ya que los items est�n encima
                if (doEdgeMenusStuff) {
                    // Cerramos los men�s no locked
                    closeNonLockedMenus(false, true, true);
                    delayTime = 0;
    ...
    /**
     * Indica si el mouse est� en un item, devuelve el n�mero del mismo o -1 en caso de no estar
     *
     * @param x
     * @param y
     * @return devuelve el n�mero del item o -1 en caso de no estar
     */
    ...
    /**
     * Indica si el mouse est� en un item del submenu de abajo, devuelve el n�mero del mismo o -1 en caso de no estar
     *
     * @param x
     * @param y
     * @return devuelve el n�mero del item o -1 en caso de no estar
    ...
    private int isMouseOnMenuItems(int x, int y) {
        if (y >= menuPanelPoint.y && y < (menuPanelPoint.y + MENU_PANEL_HEIGHT) && x >= menuPanelPoint.x && x < (menuPanelPoint.x + MENU_PANEL_WIDTH)) {
            Point point;
            bucle1:
            for (int y1 = 0; y1 < MENU_PANEL_NUM_ITEMS_Y; y1++) {
    ...
    /**
     * Indica si el mouse est� en un item (o en los +/-) del panel de producci�n
     *
     * @param x
     * @param y
     * @return Un punto, X es el MOUSE_ID y Y indica la posici�n del item en el array correspondiente
     */
    private Point isMouseOnProductionItems(int x, int y) {
        if (y >= productionPanelPoint.y && y < (productionPanelPoint.y + PRODUCTION_PANEL_HEIGHT) && x >= productionPanelPoint.x && x < (productionPanelPoint.x + PRODUCTION_PANEL_WIDTH)) {
            Point point;
            bucle1:
            for (int y1 = 0; y1 < PRODUCTION_PANEL_NUM_ITEMS_Y; y1++) {
    ...
    /**
     * Indica si el mouse est� en un item (o en los up/down) del panel de prioridades
     *
     * @param x
     * @param y
     * @return Un punto, X es el MOUSE_ID y Y indica la posici�n del item en el array correspondiente
    ...
    /**
     * Indica si el mouse est� en un item (o en los up/down) del panel de trade
     *
     * @param x
     * @param y
     * @return Un punto, X es el MOUSE_ID y Y indica la posici�n del item en el array correspondiente
    ...
    /**
     * Indica si el mouse est� en un item (o en los up/down) del panel de pila
     *
     * @param x
     * @param y
     * @return Un punto, X es el MOUSE_ID y Y indica la posici�n del item en el array correspondiente
    ...
    /**
     * Indica si el mouse est� en un item (o en los up/down) del panel de profesiones
     *
     * @param x
     * @param y
     * @return Un punto, X es el MOUSE_ID y Y indica la posici�n del item en el array correspondiente
    ...
    /**
     * Retorna una lista de IDs de livings a partir de lo que est� mostrando el living panel
     *
     * @return
     */
    private ArrayList<Integer> getLivings() {
        if (getLivingsPanelActive() == LIVINGS_PANEL_TYPE_CITIZENS) {
            if (livingsPanelCitizensGroupActive == -1) {
                ...
                // Est� mostrando un grupo, miramos los miembros que tiene
    ...
    /**
     * Retorna la primera posici�n de la p�gina que est� mostrando el living panel
     *
     * @return
     */
    ...
    public void mousePressed(int x, int y, int mouseButton) {
        ...
        if (isProfessionsPanelActive()) {
            if (iPanel == MOUSE_PROFESSIONS_PANEL_BUTTONS_CLOSE) {
                ...
                // Ha clicado en un item, vamos a ver qu� pasa
                if (menuProfessions == null) {
                    return;
                }

                if (mouseButton == 1) { // Bot�n derecho (back al men�)
            ...
            if (iPanel == MOUSE_PROFESSIONS_PANEL) {
                if (mouseButton == 1) { // Bot�n derecho (cerramos o tiramos 1 atr�s el men�)
                    if (menuProfessions != null) {
                        if (menuProfessions.getParent() != null) {
                            menuProfessions = menuProfessions.getParent();
        ...
        if (isMessagesPanelActive()) {
            ...
            if (iPanel == MOUSE_MESSAGES_PANEL_BUTTONS_CLOSE) {
                ...
                if (mouseButton == 1) { // Bot�n derecho (cerramos)
                    setMessagesPanelActive(-1);
                } else {
            ...
            if (mi != null && mi instanceof MilitaryItem) {
                if (World.getCell(mi.getCoordinates()).getAstarZoneID() == iASZID) {
                    ...
                    if (imi.getLocation() == iLocation) {
                        // Lo metemos en la posici�n correcta, ordenado por item level
    ...
    private void createMenuPanel(SmartMenu menu) {
        ...
        // Minibot�n para abrir/cerrar el men�
    ...
    private void createMessagesPanel() {
        ...
        // Posici�n de iconos (los 4 de arriba) dentro del panel (va aqu� pq tienen que centrarse con el subpanel)
        ...
        // �sto es para que parta los messages render
    ...
    private void createMatsPanel() {
        ...
        // Posici�n de iconos (los X de arriba) dentro del panel (va aqu� pq tienen que centrarse con el subpanel)
    ...
    private void createPrioritiesPanel() {
        ...
        // Miramos la separaci�n entre items
        ...
        // Tenemos el tama�o de los items
        ...
        // N�mero de columnas para que quepa
        ...
        if (PRIORITIES_PANEL_NUM_ITEMS > 1 && PRIORITIES_PANEL_HEIGHT > iMaxHeight) {
            if (iMaxHeight - 2 * PIXELS_TO_BORDER != 0) { // Check divisi�n por 0
                iNumColumns = PRIORITIES_PANEL_HEIGHT / (iMaxHeight - 2 * PIXELS_TO_BORDER); // Realmente no entiendo el 2*PIXELS en esta operaci�n
