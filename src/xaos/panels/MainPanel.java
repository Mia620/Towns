...
public final class MainPanel {
    ...
    public static void render() {
        ...
        // Informaci�n tooltip s�lo si no hay un men� contextual abierto
    ...
    public static int renderAllTerrains(int zView, int cellXMin, int cellXMax, int cellYMin, int cellYMax, int iBaseXGeneral, int iBaseYGeneral, Point3D pointTileMouse, int zLevelOffset, int currentTextureID) {
        ...
        Cell cell;
        forPrincipal:
        for (int i = cellXMax; i >= cellXMin; i--) {
...
//						UtilsGL.unsetColor ();
                ...
                // Under
                if (cell == null || cell.isShouldPaintUnder() || (flatMouseON && zLevelOffset == 0 && pointTileMouse != null && isMouseNearCell(cell, pointTileMouse))) { // El zLevelOffset es para que se vea bien cuando acercas al rat�n a un item con _block
        ...
        for (int i = cellXMax; i >= cellXMin; i--) {
            for (int j = cellYMin; j <= cellYMax; j++) {
                ...
                if (i >= 0 && j >= 0 && i < World.MAP_WIDTH && j < World.MAP_HEIGHT && iXGeneral > -Tile.TERRAIN_ICON_WIDTH && iYGeneral < (renderHeight + Tile.TERRAIN_ICON_HEIGHT)) {
                    ...
                    // Si la celda tiene �rdenes pintamos su tile
                    ...
                    if (gridON && zLevelOffset == 0 && cell.isDiscovered() && cell.isMined()) {
                        if (!cell.isDigged()) {
                            ...
                            // Celda digada, pero miramos si hay algo debajo (simplemente mirando el ASZID bastar�)
    ...
    /**
     * Devuelve la Z M�xima donde se deber� dibujar el 3D mouse
     *
     * @param x
     * @param y
     * @param z
    ...
    public static void renderTask(int zView, int iBaseXGeneral, int iBaseYGeneral, Point3D pointTileMouse, int currentTextureID, int cellXMin, int cellYMin) {
        ...
        if (pointTileMouse != null) {
            if (Game.getCurrentState() == Game.STATE_CREATING_TASK && (Game.getCurrentTask().getState() == Task.STATE.CREATING_ENDZONE || Game.getCurrentTask().getState() == Task.STATE.CREATING_SINGLEPOINT)) {
                ...
                // Miramos donde est� el rat�n para mostrar info por pantalla y el cuadradito blanco
                ...
                if (Game.getCurrentTask().getState() == Task.STATE.CREATING_ENDZONE) {
                    ...
                    if (Game.getCurrentTask().getType() == Task.TYPE.BUILD || Game.getCurrentTask().getType() == Task.TYPE.CREATE_AND_PLACE || Game.getCurrentTask().getType() == Task.TYPE.QUEUE_AND_PLACE) {
                        ...
                        Log.error(Messages.getString("MainPanel.3") + Game.getCurrentTask().getType() + "]", "MainPanel"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
                    }
                }
                ...
                if (bBuilding) {
                    ...
                    if (queuing) {
                        // En caso de colas el item est� en el par�metro 2
                        imi = ItemManager.getItem(Game.getCurrentTask().getParameter2());
                    } else {
                        imi = ItemManager.getItem(Game.getCurrentTask().getParameter());
                    }
                    if (imi == null) {
                        // No ha encontrado el item??? Quiz� es un createItemByType y el valor es un type, as� que pillamos el primer item con ese type
                        if (queuing) {
                            // En caso de colas el item est� en el par�metro 2
                        ...
                        if (mouseOK) {
                            ...
                            if (bBuilding) {
                                ...
                                if (!Item.isCellAvailableForItem(imi, i, j, z3D, true, true)) {
                                    ...
                                    // El item puede ir ah�, pero miraremos que no est� construyendo en el aire y no haya camino posible
                                    if (imi.canBeBuiltOnHoles()) {
                                        // Miramos si hay alg�n ASZID distinto de -1 en un radio de 3x3x3
                                        boolean bAllUnavailable = true;
                                        foriteming:
                                        for (int itemX = (i - 1); itemX <= (i + 1); itemX++) {
    ...
    private static int renderLivings(Cell cell, int iXGeneral, int iYGeneral, int currentTextureID, float fColorShadowLight, boolean bLight, int zLevelOffset) {
        ...
        for (LivingEntity alLiving : alLivings) {
            ...
            if (lemi.getType() == LivingEntity.TYPE_CITIZEN) {
                ...
                // Miramos si tiene que mostrar el signo de exclamaci�n
                ...
                if (cit.getCitizenData().getBlinkAnimationTurns() > (CitizenData.MAX_BLINK_ANIMATION_TURNS / 2)) {
                    ...
                    if (tileTask != null) {
                        ...
                        // En el caso de comer, miramos si est� pasando hambre porque no hay comida
                ...
                if (lemi.isFacingDirections()) {
                    ...
                    // Miramos si hay que hacer flip seg�n la direcci�n donde mire
...
//						iXSpecific = iXGeneral + (int) le.getPositionOffset ().x;
    ...
    private static int renderCarrying(Cell cell, Citizen cit, int iXGeneral, int iYGeneral, int currentTextureID, float fColorShadowLight, boolean bLight, int iDepth, int zLevelOffset) {
        ...
        // Miramos si est� cargando algo para dibujarlo
    ...
    /**
     * Retorna la celda en la que el mouse est� apuntando. Se le pasa la view
     *
     * @param x
     * @param y
     * @param xView
    ...
    private static Point3D getTileMouse(int x, int y, int xView, int yView, int zView) {
        // Mouse en ning�n panel (o sea, en la main area)
    ...
    /**
     * Cambia la textura Y a�adi brillo
     *
     * @param iTexture private static void unsetBrightNess (int iTexture) { //UtilsGL.glEnd (); // GL11.glBindTexture (GL11.GL_TEXTURE_2D, iTexture); // GL11.glTexEnvf (GL11.GL_TEXTURE_ENV, GL11.GL_TEXTURE_ENV_MODE, GL11.GL_MODULATE); // GL11.glTexEnvf (GL11.GL_TEXTURE_ENV, GL13.GL_COMBINE_RGB, GL11.GL_MODULATE); //UtilsGL.glBegin (GL11.GL_QUADS); }
     */
    public static void resize(int renderWidth, int renderHeight) {
        ...
        // Obtenemos el n�mero de tiles m�ximo que caben por pantalla
    ...
    /**
     * Devuelve un contextmen�segun la casilla en la que est�
     *
     * @param x
     * @param y
     * @return
    ...
    public ContextMenu getContextMenu(int x, int y) {
        ...
        // Segun lo que haya en el tile cargamos un men� u otro
