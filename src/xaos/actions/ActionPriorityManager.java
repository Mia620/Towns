...
public class ActionPriorityManager {
    ...
    private static HashMap<String, Integer> prioritiesValues; // Prioridad num�rica de cada "item" (se corresponde con la posici�n en la lista, se usa para performance)
    ...
    private static void loadXMLPriorityActions(String sXMLName) {
        try {
            ...
            // Lo recorremos entero y vamos a�adiendo a la hash
            ...
            Log.log(Log.LEVEL_ERROR, Messages.getString("ActionManager.0") + sXMLName + "] [" + e + "]", "ActionPriorityManager"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
            Game.exit();
        }
    }
    ...
    public static void load(ObjectInputStream ois) throws Exception {
        ...
        // Si hay m�s prioridades, las metemos
