...
public class SkillManager {
    ...
    private static void loadItems() {
        ...
        if (!alMods.isEmpty()) {
            for (String alMod : alMods) {
                String sModActionsPath = fUserFolder.getAbsolutePath() + FileSystems.getDefault().getSeparator() + Game.MODS_FOLDER1 + FileSystems.getDefault().getSeparator() + alMod + FileSystems.getDefault().getSeparator() + Towns.getPropertiesString("DATA_FOLDER") + "skills.xml"; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$
    ...
    private static void loadXMLSkills(String sXMLName, boolean bLoadingMain) {
        try {
            ...
            Log.log(Log.LEVEL.ERROR, Messages.getString("SkillManager.0") + e + "]", "SkillManager"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
            Game.exit();
        }
    }

    private static ArrayList<SkillEffectItem> loadEffectData(NodeList nodeList) throws Exception {
        ArrayList<SkillEffectItem> alEffects = new ArrayList<>();
